...
// Catalogo global (sin empresa_id) -- ver antecedentesPatologicos.controller.js
// y categoriasAntecedentes.routes.js para el gateo de permisos
...
async function crear(req, res, next) {
  try {
    ...
    const { rows } = await pool.query(
      `insert into categorias_antecedentes (nombre, orden, activo)
...
async function actualizar(req, res, next) {
  try {
    const { nombre, orden, activo } = req.body;
    const { rows } = await pool.query(
      `update categorias_antecedentes set
...
async function eliminar(req, res, next) {
  try {
    const { rowCount } = await pool.query('delete from categorias_antecedentes where id = $1', [req.params.id]);
    if (!rowCount) return res.status(404).json({ mensaje: 'Categoria no encontrada' });
    res.status(204).send();
  } catch (err) {
    if (err.code === '23503') return res.status(409).json({ mensaje: 'No se puede eliminar: tiene antecedentes registrados en esta categoria.' });
    next(err);
  }
}
