...
-- ---------------------------------------------------------
-- Tabla: especialidades_empresas. Representa que especialidades
-- GLOBALES tiene "activadas" cada clinica para su propio uso en
-- Citas/Campanas -- agregar una especialidad a una clinica significa
-- activar una del catalogo global, no crear un nombre nuevo privado
-- (las especialidades privadas ya existentes no pasan por aqui, viven
-- directo en especialidades.empresa_id).
-- ---------------------------------------------------------
...
-- al doctor en las comunicaciones existentes (Citas, historial clinico).
...
-- ---------------------------------------------------------
-- Tabla: politica_password. Singleton (una sola fila, id=1) editable
-- solo por super admin -- ver politicaPassword.controller.js. Los
-- valores por defecto reproducen el comportamiento historico (minimo 6,
-- sin requisitos de caracteres, pista minima 4, similitud maxima 70%).
-- ---------------------------------------------------------
